...
LQ_EXTERN_C int LQ_CALL LqTimeStrToLocTm(const char* Str, struct tm* Result) {
    ...
    if(Str[n] != ' ') return -1;
    n++;
    if((t = LqStrToInt(&OutTm.tm_mday, Str + n, 10)) < 2) return -1;
    n += t;
    if(Str[n] != ' ') return -1;
    n++;
    if((t = LqStrToInt(&OutTm.tm_hour, Str + n, 10)) < 2) return -1;
    n += t;
    if(Str[n] != ':') return -1;
    n++;
    if((t = LqStrToInt(&OutTm.tm_min, Str + n, 10)) < 2) return -1;
    n += t;
    if(Str[n] != ':') return -1;
    n++;
    if((t = LqStrToInt(&OutTm.tm_sec, Str + n, 10)) < 2) return -1;
    n += t;
    if(Str[n] != ' ') return -1;
    n++;
    if((t = LqStrToInt(&OutTm.tm_year, Str + n, 10)) < 1) return -1;
    n += t;
    OutTm.tm_year -= 1900;
...
LQ_EXTERN_C LqTimeMillisec LQ_CALL LqTimeGetMaxMillisec() {
    return LQ_NUMERIC_MAX(LqTimeMillisec);
}
