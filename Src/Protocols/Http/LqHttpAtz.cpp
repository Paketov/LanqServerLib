...
void LQ_CALL LqHttpMdlHandlersNonce(LqHttpConn* c, char* MethodBuf, size_t MethodBufSize)
{
    ...
    {
        LastCheck = sec;
        Random = (((ullong)rand() << 32) | (ullong)rand()) + ms % 50000;
    }
    char IpBuf[100];
    IpBuf[0] = '\0';
    ...
    for(const char* k = IpBuf; *k != '\0'; k++)
        h = 63 * h + *k;
    snprintf(MethodBuf, MethodBufSize, "%llx", h);
}
...
static bool LqHttpAtzGetBasicBase64LoginPassword(char* Str, size_t Len, char** Res, size_t* ResLen)
{
    ...
    Str[Len] = '\0';
    int a = -1, b = -1;
    sscanf(Str, "Basic %n%*[a-zA-Z0-9/+=]%n", &a, &b);
    Str[Len] = t;
    if((a != -1) && (b != -1))
    {
        *Res = Str + a;
...
static char* LqHttpAtzGetBasicCode(const char* User, const char* Password)
{
    ...
    if(d == nullptr)
        return nullptr;
    d[0] = '\0';
    snprintf(d, l, "%s:%s", User, Password);
    size_t ld = (size_t)((float)l * 1.4f) + 2;
    char* d2 = (char*)calloc(ld, 1);
    if(d2 == nullptr)
    {
        free(d);
        return nullptr;
    }
    LqBase64Code(d2, d, LqStrLen(d));
    free(d);
    return d2;
}
...
{
    LqHttpAtz* r = LqFastAlloc::New<LqHttpAtz>();
    if(r == nullptr)
        return nullptr;
...
LQ_EXTERN_C bool LQ_CALL LqHttpAtzAdd(LqHttpAtz* NetAutoriz, uint8_t AccessMask, const char* UserName, const char* Password)
{
    ...
    } else if(NetAutoriz->AuthType == LQHTTPATZ_TYPE_DIGEST)
    {
        ...
        LqMd5 h;
        LqMd5Final((unsigned char*)&h, &ctx);
        LqMd5ToString(NetAutoriz->Digest[NetAutoriz->CountAuthoriz].DigestLoginPassword, &h);
        NetAutoriz->CountAuthoriz++;
    }
    LqHttpAtzUnlock(NetAutoriz);
    return true;
...
{
    if(NetAutoriz == nullptr) return false;
    LqHttpAtzLockWrite(NetAutoriz);
    if(NetAutoriz->AuthType == LQHTTPATZ_TYPE_BASIC)
    {
        ...
        for(size_t i = 0; i < nc;)
        {
            auto lp = LqBase64DecodeToStlStr(NetAutoriz->Basic[i].LoginPassword, LqStrLen(NetAutoriz->Basic[i].LoginPassword));
            if(LqStrSameMax(lp.c_str(), UserName, l))
            {
