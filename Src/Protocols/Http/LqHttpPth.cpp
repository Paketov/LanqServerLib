...
{
	NameDomain = LqStrDuplicate(Name);
	NameHash = StringHash<decltype(NameHash)>(Name);
	return true;
}
size_t LqHttpDomainPaths::IndexByKey(const char* Key, size_t MaxCount) { return StringHash<decltype(NameHash)>(Key) % MaxCount; }
size_t LqHttpDomainPaths::IndexInBound(size_t MaxCount) const { return NameHash % MaxCount; }
bool LqHttpDomainPaths::CmpKey(const char* Key) const { return LqStrSame(Key, NameDomain); }
...
)
{
	...
	auto sd = FileSystem.t.Search(WebDomenSource);
	auto dd = FileSystem.t.Search(WebDomenDest);
	if((sd == nullptr) || (dd == nullptr))
	{
		r = LQHTTPPTH_RES_NOT_HAVE_DOMEN;
		break;
	}
	auto Path = sd->t.Search(WebPath);
	if(Path == nullptr)
	{
		r = LQHTTPPTH_RES_NOT_HAVE_PATH;
		break;
	}
	r = LqHttpPthRegisterNative(dd, Path->p);
	LQ_BREAK_BLOCK_END
...
)
{
	...
	auto v = d->t.Search(WebPath);
	if(v == nullptr)
	{
		r = LQHTTPPTH_RES_NOT_HAVE_PATH;
		break;
	}
	v->p->ParentModule->UnregisterPathFromDomenProc(v->p, WebDomen);

	auto OldTableElement = d->t.RemoveRetPointer(WebPath);
	LqHttpPthRelease(OldTableElement->p);
	OldTableElement->p = nullptr;
	d->t.DeleteRetPointer(OldTableElement);

	r = LQHTTPPTH_RES_OK;
	if((size_t)(d->t.Count() * 1.7f) < d->t.AllocCount())
		d->t.ResizeAfterRemove();
	LQ_BREAK_BLOCK_END
	FileSystem.l.UnlockWrite();
	return r;
}
...
LQ_EXTERN_C LqHttpPthResultEnm LQ_CALL LqHttpPthDmnDelete(LqHttpProtoBase* Reg, const char* WebDomen)
{
	...
	{
		d->t.EnumDelete(
			[](void* DomainName, LqHttpDomainPaths::Element* e)
			{
...
/*
* Note!
* Argument @Path - not thread safe!
* if @Path use of multiple thread, then copy this string to temp local buffer before call.
...
)
{
	...
	for(l = LqStrLen(Path) - 1; l >= 0; l--)
		if(Path[l] == '/')
		{
			...
			if(p != nullptr)
			{
				if((k > 1) && !(p->p->Type & LQHTTPPTH_FLAG_SUBDIR))
					continue;
				break;
			}
		}
...
static LqHttpPth* LqHttpPthGetByAddress(LqHttpProtoBase* Reg, const char* Domain, char* Path)
{
	...
	if(p != nullptr)
	{
		LqHttpPthAssign(p->p); //Occupy path struct
		FileSystem.l.UnlockRead();
		return p->p;
	} else
	{
...
static LqHttpPthResultEnm LqHttpPthRegisterNative(LqHttpFileSystem& FileSystem, const char* WebDomen, LqHttpPth* Path)
{
	...
	auto d = FileSystem.t.Search(WebDomen);
	if(d != nullptr)
		Result = LqHttpPthRegisterNative(d, Path);
	FileSystem.l.UnlockWrite();
	return Result;
}
...
{
	auto Query = &c->Query;
	auto reg = LqHttpGetReg(c);
	auto CountSubDirs = 0U;
	if(Query->Host != nullptr)
	{
		char HostBuf[LQHTTPPTH_MAX_DOMEN_NAME];
		if(auto r = LqStrUtf8ToLower(HostBuf, sizeof(HostBuf) - 1, Query->Host, Query->HostLen))
		{
			if(r >= (HostBuf + sizeof(HostBuf)))
			{
				c->Pth = nullptr;
		...
		c->Pth = LqHttpPthGetByAddressSubdirCheck(&reg->Base, HostBuf, Query->Path, &CountSubDirs);
		Query->Path[Query->PathLen] = t2;
	} else
	{
		char t = Query->Path[Query->PathLen];
		Query->Path[Query->PathLen] = '\0';
		c->Pth = LqHttpPthGetByAddressSubdirCheck(&reg->Base, "*", Query->Path, &CountSubDirs);
