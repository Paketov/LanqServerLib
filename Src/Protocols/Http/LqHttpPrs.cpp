...
lblRep:
		for(; isalnum(*c) || (*c == '-'); c++);
		if(*(c - 1) == '-')
			return READ_URL_ERR_SYMBOLIC_HOST_NAME;
		if((*c == '.') && isalnum(c[1]))
		{
			c++;
			goto lblRep;
	...
	} else if(*c == '[')
	{
		...
		if(*c == ':')
		{
			if(c[1] == ':')
			{
				...
				return READ_URL_ERR_IPv6_HOST_NAME;
		}
		while(true)
		{
			if(IS_HEX(c))
			{
				...
				char* t = c++;
				for(; IS_HEX(c); c++);
				if(c > (t + 4)) return READ_URL_ERR_IPv6_HOST_NAME;
				if(*c == ':')
				{
					c++;
					if(*c == ':')
...
lblPort:
	...
	if(*c == ':')
	{
		...
		for(char* m = c + 5; IS_DIGIT(c) && (c < m); c++)
			d = d * 10 + (*c - '0');
		if(d > 65535) return READ_URL_ERR_PORT;
		StartPort = t + 1;
		EndPort = c;
	}
lblDir:
	...
	if(*c == '?')
	{
		...
		for(char *StartKey, *EndKey, *StartVal, *EndVal;;)
		{
			...
			if((EndKey = c) == StartKey) return READ_URL_ERR_QUERY;
			if(*c == '=')
			{
				StartVal = ++c;
				for(; IS_URIC_WITHOUT_EQ_AMP(c); c++);
...
)
{
	...
	switch(*c)
	{
		case ' ': case '\t':
		{
			if(StartMethod == c) return READ_START_LINE_ERR;	//If len method eq. 0
			eMethod = c;
			sMethod = StartMethod;
			c++;
			break;
		}
		default: return READ_START_LINE_ERR;
	}
	for(; (*c == ' ') || (*c == '\t'); c++);
	sUri = c;
	for(;; c++)
		...
		switch(*c)
		{
			...
			case '\r':
				if(c[1] == '\n')
				{
					*c = '\0';
					End = c + 2;
					return READ_START_LINE_SUCCESS;
				} else if(c[1] != '\0') 
					return READ_START_LINE_ERR;
		}
	}
lblOutLoop:
	if(c == sUri) return READ_START_LINE_ERR;
	eUri = c;
	c++;
	for(;; c++)
	{
		switch(*c)
		{
			...
			case 'H':
			{
				...
				sVer = (c += 5);
				for(; (*c >= '0') && (*c <= '9') || (*c == '.'); c++);
				if(sVer == c) return READ_START_LINE_ERR;
				for(; (*c == ' ') || (*c == '\t'); c++);
				if(*c != '\r') return READ_START_LINE_ERR;
				eVer = c;
				if(*++c != '\n') return READ_START_LINE_ERR;
				End = c + 1;
				return READ_START_LINE_SUCCESS;
...
LqHttpPrsHdrStatEnm LqHttpPrsHeader(char* String, char*& KeyStart, char*& KeyEnd, char*& ValStart, char*& ValEnd, char*& End)
{
	...
	if((c[0] == '\r') && (c[1] == '\n')) return READ_HEADER_END;

	for(; (*c == ' ') || (*c == '\t'); c++);
	StartKey = c;
	for(; ((*c >= 'a') && (*c <= 'z')) || ((*c >= 'A') && (*c <= 'Z')) || (*c == '-'); c++);
	if(StartKey == c) return READ_HEADER_ERR;
	EndKey = c;

	for(; (*c == ' ') || (*c == '\t'); c++);
	if(*c != ':') return READ_HEADER_ERR;
	c++;
	for(; (*c == ' ') || (*c == '\t'); c++);

	StartVal = c;
	for(; (*c != '\r') && (*c != '\n') && (*c != '\0'); c++);
	if(StartVal == c) return READ_HEADER_ERR;
	EndVal = c;
	if((c[0] != '\r') || (c[1] != '\n')) return READ_HEADER_ERR;

	End = c + 2;
	KeyStart = StartKey, KeyEnd = EndKey, ValStart = StartVal, ValEnd = EndVal;
	return READ_HEADER_SUCCESS;
}
